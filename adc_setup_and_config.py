...
class ADCconfig:
    ...
    adc_fw_ctrl_reg_map = {
        'adr_adc_pllen'  : 0x0033,
        'adr_adc_calstat': 0x0032,
        'adr_adc_syncse' : 0x0031,
        'adr_adc_caltrig': 0x0030,
        'adr_adc_spisel' : 0x000D,
        'adr_adc_pdwn'   : 0x000C}
    
    def __init__(self):
        #instance to host the ADC config SPI bus
        self.spi = didaq_adc_spi.DidaqSPIHost()
        #instance to host the gpio config of ADCs
        self.fpga = didaq.Didaq() #spi link to sbc
        self.num_adcs = 6
    ...
    def adcSpiBusSel(self, bus):
        ...
        '''
        self.fpga.write(self.adc_fw_ctrl_reg_map['adr_adc_spisel'], [0x00,0x00,0x00, 0xFF & bus])
        self.cur_adc = bus
    ...
    def adcPllEn(self, pll_en_mask):
        ...
        '''
        self.fpga.write(self.adc_fw_ctrl_reg_map['adr_adc_pllen'], [0x00,0x00,0x00, 0x3 & pll_en_mask])
    
    def adcPd(self, pd_mask):
        ''' adc powerdown control
        '''
        regval = self.fpga.read(self.adc_fw_ctrl_reg_map['adr_adc_pdwn'])[2:]
        regval[3] = 0x3F & pd_mask
        self.fpga.write(self.adc_fw_ctrl_reg_map['adr_adc_pdwn'], regval)
